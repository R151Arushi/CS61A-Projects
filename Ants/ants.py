...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def find_bee(nearest_place, distance):
            if not nearest_place.is_hive and nearest_place.bees and (self.min_range <= distance <= self.max_range):
                return nearest_place
            else:
                if not nearest_place.entrance.is_hive:
                    return find_bee(nearest_place.entrance, distance + 1)

        bee_place = find_bee(self.place, 0)
        if bee_place:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        bee_pl = iter(self.place.bees[:])
       
        for bee in bees_place:
            x = next(bee_pl)
            x.reduce_health(damage)
...
# END Problem 7
    ...
    def action(self, gamestate):
        ...
        elif self.place.bees and self.chew_countdown == 0:
            
            self.chew_countdown = self.chew_duration
            food = random_bee(self.place.bees)
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if insect.place == self and not insect.is_waterproof:
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        # queen_map = [isinstance(gamestate.places[x].ant, QueenAnt) for x in gamestate.places]
        if gamestate.has_queen_ant:
            return None
        gamestate.has_queen_ant = True
    ...
    def action(self, gamestate):
        ...
        while behind:
            if behind.ant:
                ...
                if behind.ant.is_double == False:
                    behind.ant.damage *= 2
                    behind.ant.is_double = True
                    
                if behind.ant.is_container:
                    if behind.ant.ant_contained:
                        if behind.ant.ant_contained.is_double == False:
                            behind.ant.ant_contained.damage *= 2
                            behind.ant.ant_contained.is_double = True
            behind = behind.exit
...
class Bee(Insect):
    ...
    def __init__(self,health, slowtimer = 0, backwards = 0):
        super().__init__(health)
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        if self.scared_state and not self.is_brave:
            destination = next(self.scare(gamestate))
        elif self.slow_state:
            destination = next(self.slow(gamestate))
        else:
    ...
    def slow(self, gamestate):
        """Slow the bee for a further LENGTH turns."""
        # BEGIN Problem EC

        while self.slowtimer > 0: 
            if gamestate.time % 2 == 0:       
               yield self.place.exit               
            else:
                yield self.place   
        self.slow_state = False
        yield self.place.exit
    ...
    def scare(self, gamestate):
        ...
        if not self.is_brave:
            while self.backwards > 0: 
                if gamestate.time % 2 == 0:
                    self.backwards -= 1
                    if self.backwards == 0:
                            self.is_brave = True
                    if not self.place.entrance.is_hive:
                        yield self.place.entrance
                    else:
                        yield self.place         
                else:
                    if self.slow_state or self.place.entrance.is_hive:
                        yield self.place  
            yield self.place.exit
                #time += 1
            self.is_brave = True  
