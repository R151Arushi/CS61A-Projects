...
def about(topic):
    ...
    def is_relevant(paragraph):
        no_punctuation_lower_paragraph = ' ' + remove_punctuation(lower(paragraph)) + ' '
        for word in topic:
            if ' ' + word + ' ' in no_punctuation_lower_paragraph:
                return True
        return False
    return is_relevant
...
def accuracy(typed, reference):
    ...
    if typed_words == reference_words:
        return float(100)
    if typed_words and not reference_words or reference_words and not typed_words:
        return float(0)
    i = 0
    num_correct = 0
    # difference = abs(len(typed_words) - len(reference_words))
    for word in range(min(len(typed_words), len(reference_words))):
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    similar_words = [word for word in valid_words if diff_function(word, typed_word, limit) <= limit]
    if len(similar_words) > 0:
        closest_word = similar_words[0]
        for word in similar_words:
            if diff_function(word, typed_word, limit) < diff_function(closest_word, typed_word, limit):
                closest_word = word
        return closest_word
    return typed_word
    # END PROBLEM 5
...
def feline_flips(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def diff_counter(w1, w2, new_lim):
        if new_lim < 0:
            return 1
        if len(w1) == 0 or len(w2) == 0:
            return abs(len(start) - len(goal))
        if w1[0] != w2[0]:
            return 1 + diff_counter(w1[1:], w2[1:], new_lim - 1)
        else:
            return diff_counter(w1[1:], w2[1:], new_lim)

    return diff_counter(start, goal, limit)
    # END PROBLEM 6
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    while i < len(sofar) and sofar[i] == prompt[i]:
        correct_sofar += [sofar[i]]
        i += 1
    progress = len(correct_sofar) / len(prompt)
...
def time_per_word(words, times_per_player):
    ...
    for player in times_per_player:
        ...
        times = []
        while i < len(player):
            times += [player[i] - player[i-1]]
            i += 1
        times_per_word += [times]   # list containing a list
...
def fastest_words(match):
    ...
    for word in word_indices:
        quickest_word_player = 0
        for player in player_indices:
            if players_times[player][word] < players_times[quickest_word_player][word]:
                    quickest_word_player = player
        if quickest_word_player == 0:
            quickest_words[quickest_word_player] += [word_at(match, word)]
        else:
            quickest_words[quickest_word_player] += [word_at(match, word)]
    return quickest_words
